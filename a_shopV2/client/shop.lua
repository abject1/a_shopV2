...
OpenAction = function(action, shelf, text)
    if action["value"] == "checkout" then
        if payAmount > 0 and #Basket then
            ...
            pNotify("Vous n'avez rien dans votre panier!", 'error', 1500)
        end
    else
...
CashRegister = function(titel)
        local elements = {
            {label = '<span style="color:lightgreen; border-bottom: 1px solid lightgreen;">Confirmer</span>', value = "Oui"},
            {label = 'Total à payer: <span style="color:green">$' .. payAmount ..'</span>'},
        }
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                ...
                if data.current.value == "Oui" then
                    menu.close()
                    ESX.UI.Menu.Open(
                        'default', GetCurrentResourceName(), 'penis2',
                        {
                            ...
                            align    = 'center',
                            elements = {
                                {label = "Payer en cash", value = "cash"},
                                {label = "Payer avec carte de crédit", value = "bank"},
                            },
                        },
                        function(data2, menu2)
                            ESX.TriggerServerCallback('a_shop:CheckMoney', function(hasMoney)
                                if hasMoney then
                                    TriggerServerEvent('a_shop:Cashier', payAmount, Basket, data2.current["value"])
                                    payAmount = 0
                                    Basket = {}
                                    menu2.close()
                                else
                                    pNotify("Tu n'a pas assez d'argent!", 'error', 1500)
                                end
...
ShelfMenu = function(titel, shelf)
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            pNotify("Ajout de " .. data.current.value .. " de " .. data.current["realLabel"] .. " dans ton panier", 'alert', 1500)           
        end,
    function(data, menu)
        menu.close()
    end)
...
OpenBasket = function()
    if payAmount > 0 and #Basket then
        local elements = {
            {label = 'Total à payer: <span style="color:green">$' .. payAmount},
        }
        for i=1, #Basket do
            local item = Basket[i]
            table.insert(elements, {
        ...
        ESX.UI.Menu.Open(
            ...
            function(data, menu)
                if data.current.value == 'empty' then
                    ...
                    payAmount = 0
                    menu.close()
                    pNotify("Tout à été retiré de ton panier.", "error", 2500)
                end
                    ...
                    ESX.UI.Menu.Open(
                        ...
                        function(data2, menu2)
                            if data2.current["value"] == "deleteItem" then
                                pNotify("Vous avez retiré " .. Basket[index]["amount"] .." ".. Basket[index]["label"] .. " de ton panier.", "alert", 2500)
